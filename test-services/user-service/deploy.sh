...
echo "[*] Building user-service version ${APP_VERSION}"
docker build -t "user-service:${APP_VERSION}" ${BASH_SOURCE%/*}/.
...
echo "[*] Deploying user-service version ${APP_VERSION}"
kubectl apply -k "${BASH_SOURCE%/*}/deploy/base"
echo "[+] user-service version ${APP_VERSION} deployed successfully"
