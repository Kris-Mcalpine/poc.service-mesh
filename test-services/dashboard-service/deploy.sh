...
echo "[*] Building dashboard-service version ${APP_VERSION}"
docker build -t "dashboard-service:${APP_VERSION}" ${BASH_SOURCE%/*}/.
...
echo "[*] Deploying dashboard-service version ${APP_VERSION}"
kubectl apply -k "${BASH_SOURCE%/*}/deploy/base"
echo "[+] dashboard-service version ${APP_VERSION} deployed successfully"
